...
if(ig.input.state('cam_up')){this.posz++;}
if(ig.input.state('cam_down')){this.posz--;}
...
this.lastAnim=this.currentAnim;this.lastAnimTile=this.currentAnim.tile;},draw:function(){if(this.mesh){this.mesh.position.x=(this.pos.x+this.planeSize.x*0.5);this.mesh.position.y=-(this.pos.y+this.planeSize.x*0.5);this.mesh.position.z=this.posz;this.mesh2.position.x=(this.pos.x+this.planeSize.x*0.5);this.mesh2.position.y=-(this.pos.y+this.planeSize.x*0.5);}
this.parent();}});});

// lib/game/entities/node.js
ig.baked=true;ig.module('game.entities.node').requires('game.entities.3d','game.enums').defines(function(){EntityNode=Entity3d.extend({size:{x:80,y:80},offset:{x:0,y:0},collides:ig.Entity.COLLIDES.PASSIVE,zIndex:125,color:['#48cfad','#37bc9b'],nodeSize:NodeSize.SMALL,baseDefense:0,health:0,defense:0,attack:0,productionRate:0,threads:[],coreNode:true,team:null,state:NodeState.VACANT,_wmIgnore:false,_wmDrawBox:true,_wmBoxColor:0x48cfad,init:function(x,y,settings){this.parent(x,y,settings);this.baseDefense=this.nodeSize+1;this.health=this.defense=this.baseDefense;for(var i=0;i<this.nodeSize;i++){this.addThread(i);this.setThreadData(i,DataType.PRODUCTIVE,Math.random()*7);this.setThreadData(i,DataType.DEFENSIVE,Math.random()*5);this.setThreadData(i,DataType.MALICIOUS,Math.random()*4);}
this.setTeam(Math.round(Math.random()*9));},init3d:function(){this.texture=new THREE.Texture(this.imageData);this.texture.magFilter=THREE.NearestFilter;this.texture.minFilter=THREE.LinearMipMapLinearFilter;this.mesh=new THREE.Mesh(new THREE.PlaneGeometry(this.size.x,this.size.y),new THREE.MeshLambertMaterial({map:this.texture,transparent:true}));this.mesh.position.x=(this.pos.x+(this.size.x*0.5));this.mesh.position.y=-(this.pos.y+(this.size.y*0.5));this.mesh.position.z=16;var self=this;setTimeout(function(){self.texture.needsUpdate=true;ig.game.scene.add(self.mesh);},0);},generateTexture:function(){var ox=this.size.x*0.5-8;var oy=this.size.y*0.5-8;var cvs=ig.$new('Canvas');cvs.width=this.size.x;cvs.height=this.size.y;var ctx=cvs.getContext('2d');ctx.save();ctx.fillStyle=this.color[0];ctx.fillRect(ox,oy,15,15);ctx.fillStyle=this.color[1];ctx.moveTo(ox,oy+15);ctx.lineTo(ox+15,oy);ctx.lineTo(ox+15,oy+15);ctx.fill();if(false){ctx.fillStyle=this.color[0];ctx.fillRect(ox,oy,14,1);ctx.fillRect(ox+14,oy,1,14);ctx.fillRect(ox+1,oy+14,14,1);ctx.fillRect(ox,oy+1,1,14);}
...
return null;},update:function(){this.parent();var p,d,m;p=d=m=0;for(var i=0;i<this.threads.length;i++){p+=this.threads[i].data.p;d+=this.threads[i].data.d;m+=this.threads[i].data.m;}
...
// lib/game/entities/network.js
ig.baked=true;ig.module('game.entities.network').requires('game.entities.3d','game.enums').defines(function(){EntityNetwork=Entity3d.extend({size:{x:80,y:80},offset:{x:0,y:0},collides:ig.Entity.COLLIDES.PASSIVE,zIndex:125,damaged:false,direction:0,init:function(x,y,settings){this.parent(x,y,settings);this.generateTexture();},init3d:function(){this.texture=new THREE.Texture(this.imageData);this.texture.magFilter=THREE.NearestFilter;this.texture.minFilter=THREE.LinearMipMapLinearFilter;this.mesh=new THREE.Mesh(new THREE.PlaneGeometry(this.size.x,this.size.y),new THREE.MeshLambertMaterial({map:this.texture,transparent:true}));this.mesh.position.x=(this.pos.x+(this.size.x*0.5));this.mesh.position.y=-(this.pos.y+(this.size.y*0.5));this.mesh.position.z=12;var self=this;setTimeout(function(){self.texture.needsUpdate=true;ig.game.scene.add(self.mesh);},0);},generateTexture:function(){var cvs=ig.$new('Canvas');cvs.width=this.size.x;cvs.height=this.size.y;var ctx=cvs.getContext('2d');ctx.fillStyle='#242e3b';if(this.direction===NetworkDir.HORZ)ctx.fillRect(0,38,this.size.x,3);if(this.direction===NetworkDir.VERT)ctx.fillRect(38,0,3,this.size.y);if(this.direction===NetworkDir.NE){ctx.fillRect(38,38,38,3);ctx.fillRect(38,0,3,38);}
if(this.direction===NetworkDir.NW){ctx.fillRect(0,38,38,3);ctx.fillRect(38,0,3,38);}
if(this.direction===NetworkDir.SE){ctx.fillRect(38,38,38,3);ctx.fillRect(38,38,3,38);}
if(this.direction===NetworkDir.SW){ctx.fillRect(0,38,38,3);ctx.fillRect(38,38,3,38);}
if(this.direction===NetworkDir.HORZ_N){ctx.fillRect(0,38,this.size.x,3);ctx.fillRect(38,0,3,38);}
if(this.direction===NetworkDir.HORZ_S){ctx.fillRect(0,38,this.size.x,3);ctx.fillRect(38,38,3,38);}
if(this.direction===NetworkDir.VERT_E){ctx.fillRect(38,0,3,this.size.y);ctx.fillRect(38,38,38,3);}
if(this.direction===NetworkDir.VERT_W){ctx.fillRect(38,0,3,this.size.y);ctx.fillRect(0,38,38,3);}
if(this.direction===NetworkDir.CROSS){ctx.fillRect(38,0,3,this.size.y);ctx.fillRect(0,38,this.size.x,3);}
if(this.damaged){ctx.fillStyle='#f00';ctx.fillRect(37,37,5,5);}
this.imageData=cvs;console.log(this.imageData.toDataURL());},update:function(){this.parent();},draw:function(){ig.system.context.drawImage(this.imageData,ig.system.getDrawPos(this.pos.x-ig.game.screen.x),ig.system.getDrawPos(this.pos.y-ig.game.screen.y),ig.system.getDrawPos(this.size.x),ig.system.getDrawPos(this.size.y));this.parent();}});});

// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','impact.debug.debug','game.enums','game.entities.camera','game.entities.node','game.entities.network').defines(function(){__gameWidth=256;__gameHeight=192;__gameScale=4;__tileSize=8;MyGame=ig.Game.extend({scene:null,renderer:null,camera:null,ground:[],player:null,clearColor:Colors.GRAY[3],_3d:!ig.ua.iOS,font:new ig.Font('media/04b03.font.png'),init:function(){ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.W,'cam_up');ig.input.bind(ig.KEY.A,'cam_left');ig.input.bind(ig.KEY.S,'cam_down');ig.input.bind(ig.KEY.D,'cam_right');ig.input.bind(ig.KEY._1,'zoom_in');ig.input.bind(ig.KEY._2,'zoom_out');this.spawnEntity('EntityNetwork',128,128);this.spawnEntity('EntityNode',128,128);this.spawnEntity('EntityCamera',128,128);this.sortEntities();if(this._3d){this.init3d();for(var i=0;i<this.entities.length;i++)this.entities[i].init3d();}},init3d:function(){__viewAngle=40;__aspectRatio=__gameWidth/__gameHeight;__near=0.1;__far=10000;var $container=document.querySelector('body');this.renderer=new THREE.WebGLRenderer();this.renderer.setClearColor(0x171c21);this.renderer.sortObjects=false;this.camera=new THREE.PerspectiveCamera(__viewAngle,__aspectRatio,__near,__far);this.camera.rotation.x=__viewAngle*(Math.PI/180);this.camera.position.z=74;this.scene=new THREE.Scene();this.scene.add(this.camera);this.renderer.setSize(__gameWidth*__gameScale,__gameHeight*__gameScale);$container.appendChild(this.renderer.domElement);var brightness=0.95;var light=new THREE.AmbientLight(0xFFFFFF);this.scene.add(light);var levelWidth=256;var levelHeight=256;},update:function(){this.parent();if(ig.input.pressed('zoom_in')){this.camera.position.z--;console.log(this.camera.position.z);}
if(ig.input.pressed('zoom_out')){this.camera.position.z++;console.log(this.camera.position.z);}
var camera=this.getEntitiesByType('EntityCamera')[0];if(camera){this.screen.x=(camera.pos.x+(camera.size.x*0.5))-(ig.system.width*0.5);this.screen.y=(camera.pos.y+(camera.size.y*0.5))-(ig.system.height*0.5);if(this._3d&&this.camera&&camera.mesh){this.camera.position.x=camera.pos.x+camera.size.x*0.5;this.camera.position.y=-(68+camera.pos.y+camera.size.y*0.5);this.camera.position.z=74+camera.mesh.position.z;}}},draw:function(){this.parent();if(this._3d&&this.renderer){this.renderer.render(this.scene,this.camera);}}});ig.main('#canvas',MyGame,60,__gameWidth/2,__gameHeight/2,3);});