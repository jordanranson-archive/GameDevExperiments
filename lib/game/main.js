...
MyGame = ig.Game.extend({
    ...
    loadLevel: function( image ) {
        console.log( image.originalData );
        ...
        lg.Util.drawTilemap( this.level.tileMap, tilemapCtx );
        console.log( this.level, tilemapCvs.toDataURL() );

        /*this.networkChunks = [];
        var tempCvs, tempCtx;
        for( var y = 0; y < Math.ceil( tilemapCvs.height/256 ); y++ ) {
            this.networkChunks[y] = [];
            for( var x = 0; x < Math.ceil( tilemapCvs.width/256 ); x++ ) {
                tempCvs = ig.$new( 'Canvas' );
                tempCvs.width = tempCvs.height = 256;
                tempCtx = tempCvs.getContext( '2d' );
                tempCtx.drawImage( tilemapCvs, x*256, y*256, 256, 256 );
                this.networkChunks[y][x] = tempCvs;
            }
        }
