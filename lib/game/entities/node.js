...
EntityNode = Entity3d.extend({
    ...
    generateTexture: function() {
        ...
        var ox = this.size.x*0.5 - 16;
        var oy = this.size.y*0.5 - 16;
        ...
        if( this.coreNode ) {
            ...
            ctx.fillRect( ox, oy, 28, 2 );
            ctx.fillRect( ox+28, oy, 2, 28 );
            ctx.fillRect( ox+2, oy+28, 28, 2 );
            ctx.fillRect( ox, oy+2, 2, 28 );
        }
        ctx.restore();

        // For every thread
        var thread, slot, row, col, x, y, grid = [];
        ...
        for( var i = 0; i < this.threads.length; i++ ) {
            ...
            // create the grid for drawing/grouping data in clusters
