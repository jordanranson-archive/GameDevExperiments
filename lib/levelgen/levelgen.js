...
lg.Branch = ig.Class.extend({
	...
	update: function() {
		...
		if( this.steps <= 0 ) {
			...
			// choose weights
			if( this.action === 'forward' && this.lastAction[this.lastAction.length-1] === 'forward' ) this.weights[0] = 50;
			else this.weights[0] = 100;

			if( this.aliveCounter < 2 ) this.weights[1] = 0;
			else if( this.action === 'turn' && this.lastAction[this.lastAction.length-1] === 'turn' ) this.weights[1] = 50;
			else this.weights[1] = 100;
			
			if( this.aliveCounter < 2 ) this.weights[2] = 0;
			else if( this.action === 'branch' && this.lastAction[this.lastAction.length-1] === 'branch' ) this.weights[2] = 50;
		...
		if( this.lastAction.length > 8 ) this.lastAction.shift();
