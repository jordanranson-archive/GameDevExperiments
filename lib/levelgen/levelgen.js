...
lg.Levelgen = ig.Class.extend({
	...
	template: new ig.Image( 'media/templates/sun.png' ),

	init: function( context ) {
		this.context = context;
	},

	generateLevel: function( params ) {
		params = params ? params : this.params;

		var template = lg.Util.processTemplate( this.template );
...
lg.Util.generateMap = function( params, template ) {
	var map = [];
	var branches = [];
		...
		for( var y = 0; y < map.length; y++ ) {
			for( var x = 0; x < map[y].length; x++ ) {
				...
				if( !!~tile.indexOf('!') ) {
					...
					for( var i = 0; i < 4; i++ ) {
						if( !!~tile.indexOf(i) ) direction = i;
						break;
					}

					direction = direction < 0 ? Math.random()*4<<0 : direction;
					lg.Util.carve( map, x, y, direction );
...
lg.Util.processTemplate = function( image ) {
	...
	for (var y = 0; y < image.height; ++y) {
	    for (var x = 0; x < image.width; ++x) {
	        ...
	        if( rgbToHex( r, g, b ) === '333333' ) map[y][x] = '.';
	        if( rgbToHex( r, g, b ) === 'ffffff' ) map[y][x] = 'X';

	        if( rgbToHex( r, g, b ) === 'ff00ff' ) map[y][x] = '!';
	        if( rgbToHex( r, g, b ) === 'ff33ff' ) map[y][x] = '!0';
	        if( rgbToHex( r, g, b ) === 'ff66ff' ) map[y][x] = '!1';
	        if( rgbToHex( r, g, b ) === 'ff99ff' ) map[y][x] = '!2';
	        if( rgbToHex( r, g, b ) === 'ffccff' ) map[y][x] = '!3';
	    }
	}
