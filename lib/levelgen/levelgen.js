...
lg.Util.importLevel = function( image ) {
	...
	for (var y = 0; y < image.height; ++y) {
	    for (var x = 0; x < image.width; ++x) {
			...
			if( tile !== '000000' ) {
				if( y === 2 && x === 2 ) {
					console.log( tile, n, e, s, w );
				}
					 if( n === '000000' && e !== '000000' && s === '000000' && w !== '000000' ) { tileMap[y][x] = '0'; } // h
				else if( n === '000000' && e !== '000000' && s === '000000' && w === '000000' ) { tileMap[y][x] = '0'; } // h - cap
				else if( n === '000000' && e === '000000' && s === '000000' && w !== '000000' ) { tileMap[y][x] = '0'; } // h - cap
				else if( n !== '000000' && e === '000000' && s !== '000000' && w === '000000' ) { tileMap[y][x] = '1'; } // v
				else if( n !== '000000' && e === '000000' && s === '000000' && w === '000000' ) { tileMap[y][x] = '1'; } // v - cap
				else if( n === '000000' && e === '000000' && s !== '000000' && w === '000000' ) { tileMap[y][x] = '1'; } // v - cap
...
lg.Util.drawTilemap = function( map, context ) {
	var tile;
	for( var y = 0; y < map.length; y++ ) {
		for( var x = 0; x < map[y].length; x++ ) {
			tile = map[y][x];
			...
			// draw network
			context.fillStyle = '#525c6b';
			if( tile === 'X' ) context.fillRect( x*80, y*80, 80, 80 );
			if( tile === '0' ) context.fillRect( x*80, y*80+25, 80, 25 );
			if( tile === '1' ) context.fillRect( x*80+25, y*80, 25, 30 );
			if( tile === '2' ) {
				context.fillRect( x*80+25, y*80+25, 50, 25 );
				context.fillRect( x*80+25, y*80, 25, 50 );
			}
			if( tile === '3' ) {
				context.fillRect( x*80, y*80+25, 50, 25 );
				context.fillRect( x*80+25, y*80, 25, 50 );
			}
			if( tile === '4' ) {
				context.fillRect( x*80+25, y*80+25, 50, 25 );
				context.fillRect( x*80+25, y*80+25, 25, 50 );
			}
			if( tile === '5' ) {
				context.fillRect( x*80, y*80+25, 50, 25 );
				context.fillRect( x*80+25, y*80+25, 25, 50 );
			...
			if( tile === '6' ) {
				context.fillRect( x*80, y*80+25, 80, 25 );
				context.fillRect( x*80+25, y*80, 25, 50 );
			}
			if( tile === '7' ) {
				context.fillRect( x*80, y*80+25, 80, 25 );
			...
			if( tile === '8' ) {
				context.fillRect( x*80+25, y*80+25, 50, 25 );
				context.fillRect( x*80+25, y*80, 25, 80 );
			}
			if( tile === '9' ) {
				context.fillRect( x*80, y*80+25, 50, 25 );
				context.fillRect( x*80+25, y*80, 25, 80 );
			...
			// draw nodes
			context.fillStyle = '#0ff';
			if( tile === 'N' ) context.fillRect( x*80, y*80, 80, 80 );
		}
	}
};
